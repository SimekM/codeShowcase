...
final class OrderPresenter extends _basePresenter
{
    ...
    public function renderAdress(): void
    {
        $cartId = $this->loadCartInfo();
        $this->template->cartId = $cartId;
    ...
    public function sendEmail($values, $cartItems, $cartTotal, $orderId)
    {
        ...
        foreach ($cartItems as $cartItem) {
            $productsListMessage.= '<br><p>Kód položky: '.$cartItem->product_id.'</p><p>Název produktu: '.$cartItem->title_cs.'</p><p>Počet kusů: '.$cartItem->quantity.'</p><p>Cena za jeden: '.$cartItem->price.'</p><p>Celková cena produktu: '.$cartItem->price * $cartItem->quantity.'</p>';
        }

        $messageForCompany->setFrom($emailAccountInfo->userName)
        ->addTo('eshop@e-siga.cz')
        ...
        ->setHtmlBody('
            ...
            <body>
                ...
                <p>Jméno / Firma: '.$values['input-name'].'</p>
                <p>Telefon: '.$values['input-phone'].'</p>
                <p>Email: '.$values['input-email'].'</p>    
                <p>IČO: '.$values['input-ico'].'</p>
                <p>DIČ: '.$values['input-dic'].'</p>
                <p>Město: '.$values['input-city'].'</p>
                <p>Ulice: '.$values['input-street'].'</p>
                <p>PSČ: '.$values['input-psc'].'</p>                
                <br>
                <h3>Doručovací adresa</h3>
                <p>Jméno / Firma: '.$values['input-delivery-name'].'</p>
                <p>Město: '.$values['input-delivery-city'].'</p>
                <p>Ulice: '.$values['input-delivery-street'].'</p>
                <p>PSČ: '.$values['input-delivery-psc'].'</p>
                <br>
            ...
            ->setSubject('Potvrzení ojednávky SIGA')
            ->setHtmlBody('
                <html>
                    <head></head>
                <body>
