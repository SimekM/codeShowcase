...
class Cart
{
    ...
    // {        
    //     $cartItems = $this->database->fetchAll(
    //         'SELECT c.id, c.product_id, c.quantity, p.title_cs, p.img_src FROM cart_items AS c JOIN products AS p ON c.product_id = p.id WHERE c.cart_id = ?',
    //         $cartId
    //     );
    //     return $cartItems;
    // }
    ...
    public function getCartItems($cartId)
    {
        $query = "
            SELECT 
                ...
                p.img_src,
                p.price,
                tt.tag_id AS category_id
            FROM cart_items AS c
            JOIN products AS p ON c.product_id = p.id
            JOIN tags_relationships AS tr ON tr.product_id = p.id
            JOIN tags_taxonomy AS tt ON tt.id = tr.tag_id AND tt.taxonomy = 'product_category'
            WHERE c.cart_id = ?
        ";
    ...
    public function createOrder($cartId, $cartTotal, $cartItems, $values)
    {
        ...
        try {
            ...
            $orderNumber = date('Ymd') . str_pad((string)mt_rand(0, 9999), 4, '0', STR_PAD_LEFT);
            ...
            $this->database->table('order_customer_info')->insert([
                ...
                'ico' => $values['input-ico'],
                'dic' => $values['input-dic'],
                'city' => $values['input-city'],
                'street' => $values['input-street'],
                'postal_code' => $values['input-psc'],
                'delivery_name' => $values['input-delivery-name'],
                'delivery_city' => $values['input-delivery-city'],
                'delivery_street' => $values['input-delivery-street'],
                'delivery_postal_code' => $values['input-delivery-psc'],
    ...
    public function getOrderItems($orderId) {
        $query = $this->database->query("
            SELECT oi.quantity, oi.price, p.title_cs, p.id AS product_id, p.img_src
            FROM order_items oi
